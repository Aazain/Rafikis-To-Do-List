	...
	<div class="registrationBtn">
		...
		<h1 class="title"> <a href="." class="listSelect"><i class="fa fa-bars pull-left dropdown"></i></a> To Do List</h1>
				...
				{#each listData as item}
											  <div class="item">
												<li class="taskName">
													<input type="checkbox" bind:checked={item.status} on:change={()=>editStatus(item._id, item.name)} readonly name="taskCheck" class="taskComplete">
													<button disabled id="deleteBtn" class="btn btn removeButton pull-right" on:click={()=>removeFromList(item._id)}><i class="fa fa-trash w3-medium"></i></button>
													<button disabled class="editbtn btn btn pull-right" data-toggle="modal" data-target="#editorModal{item._id}"><i class="fa fa-edit w3-medium"></i></button>
		  ...
		  <div class="footer">
			  <button disabled type="submit" on:click={postToList} class="addbtn btn btn-dark">+ New Task</button> 
		  </div>
